...
void facetsToRays(perl::Object f) 
{
   ...
   for (int i=0; i<n_cones; ++i) {
      ...
      const Matrix<Coord> c_facets = zero_vector<Rational>() | (facets.minor(pos_eq,All) / -facets.minor(neg_eq,All));
      const Matrix<Coord> c_lspan = ls_exists ? zero_vector<Rational>() | linearSpan.minor(linearSpanIndices.row(i), All)
                                              : Matrix<Coord>(0, c_facets.cols());
      const auto v_description = polytope::enumerate_vertices(c_facets, c_lspan, true);
      Matrix<Coord> c_rays= v_description.first.minor(All, range_from(1));

      if (i==0) {
         lineality_space= v_description.second.minor(All, range_from(1));
