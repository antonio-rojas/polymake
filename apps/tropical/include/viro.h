...
#ifndef POLYMAKE_TROPICAL_VIRO_H
#define POLYMAKE_TROPICAL_VIRO_H
...
struct orthant_iterator {
   Vector<bool> vec; // TODO vector<char> or sth
   int ord; // TODO unsigned long or sth
   bool carry;
...
// TODO incmatrix -> vector<pair> or pair<vector,vector>
...
{
   // TODO do the reflection somewhere else?
   ...
   // TODO function to get minimum of matrix entries?
...
{
   // TODO compute viro_cells on the way
   // TODO (unsigned) long (long)?
   const int dim = monomials.cols() - 1;
   const int n_orthants = 1<<dim; // TODO use std::pow() instead?
   ...
   // build the complex:
   Matrix<Rational> points(0, vertices_moved.cols() - 1); // TODO listmatrix?
   ...
   for (auto O = orthant_iterator(dim); !O.at_end(); ++O) {
      ...
      // TODO memory vorher schon allocaten?
      ...
      for (auto c = entire(viro_cells[O.ord]); !c.at_end(); c++, current_row++) {
         ...
         for (auto cd = entire(cells.row(*c)*far_vertices_cd); !cd.at_end(); cd++) {
            ...
            for (auto o = entire(tmp); !o.at_end(); o++) {
               // TODO nonzero positionen vorher ausrechnen und merken!
