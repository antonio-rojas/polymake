...
sub new {
   ...
   if (defined($auto_func->class)
       and !grep { $_->flags & $func_is_static } @{$auto_func->arg_descrs}) {
   ...
   foreach my $arg_descr (@{$auto_func->arg_descrs}) {
      ...
      if (defined (my $typeid=get_magic_typeid($args->[$arg_no], $lval_flag))) {
         ...
         unless ($arg_descr->flags & $func_is_static) {
            $arg_type="Wary<$arg_type>" if $arg_descr->flags & $func_arg_is_wary;
            if ($typeid == $func_arg_is_const_ref) {
               $arg_type="const $arg_type&";
            } elsif ($typeid == $func_arg_is_lval_ref) {
               $arg_type="$arg_type&";
            }
            $arg_type="perl::Canned<$arg_type>";
            $arg_type="perl::AnchorArg<$arg_type>" if $arg_descr->flags & $func_arg_has_anchor;
         }
      } else {
         my $proto= $arg_descr->flags & $func_is_static
                    ? $args->[$arg_no]->type
                    : guess_builtin_type($args->[$arg_no], $arg_descr->flags & $func_arg_is_integral);
         ...
         if (ref($opts->builtin) eq "Polymake::Enum") {
            ...
            $arg_type=$opts->name;
            $arg_type="Wary<$arg_type>" if $arg_descr->flags & $func_arg_is_wary;
            $arg_type="perl::TryCanned<const $arg_type>" unless $arg_descr->flags & $func_is_static;
         } else {
            $arg_type=$opts->name;
         }
      ...
      } elsif ($arg_descr->flags & $func_is_static and defined($auto_func->class)) {
...
sub cpperl_definition {
   ...
   if ($func->flags & $func_returns_list) {
      $def->{ret}="list";
   } elsif ($func->flags & $func_returns_void) {
      $def->{ret}="empty";
   } elsif ($func->flags & $func_returns_lvalue) {
      $def->{ret}="lvalue";
   }
...
sub cpperl_define_with {
   my ($self, $args)=@_;
   my $proto=$args->[$self->arg_descrs->[0]->flags & $func_args_are_swapped ? 1 : 0]->type;
   $proto->cppoptions->finalize;
   new LackingTypeInstance($proto);
}
...
sub remove_instance {
   ...
   if ($inst_num < @{$cpperl_file->instances} &&
      ...
      $cpperl_file->instances->[$inst_num]=undef;
      $cpperl_file->needs_squeeze=1;
   } else {
