   ...
   if ($last>=0) {
      ...
      for (my $tp=0; $tp<$last; ++$tp) {
         if ($tp==$extra && defined($auto_func->class)) {
            ...
            next;
         } elsif ($tp>=$extra  &&  $auto_func->arg_descrs->[$tp-$extra]->flags & $func_arg_is_univ_ref) {
      ...
      ((($last>=$extra  &&  $last-$extra <= $#{$auto_func->arg_descrs}  &&
        $auto_func->arg_descrs->[$last-$extra]->flags & $func_arg_is_univ_ref)
...
sub parse_arg_attrs {
   ...
   if (pos($attrs) < length($attrs)) {
      if ($attrs =~ /\G & ( & (\s*const)?)? $/xg) {
         if ($1) {
            if ($arg_flags & $func_arg_has_anchor) {
               croak( "superfluous argument attribute 'anchor': implied by universal reference" );
            }
            $arg_flags |= ($2 ? $func_arg_is_const_or_rval_ref : $func_arg_is_univ_ref) | $func_arg_has_anchor;
         } else {
            $arg_flags |= $func_arg_is_lval_ref;
         }
      } else {
...
sub prepare_arg_list {
   ...
   my $static_seen = $Application::plausibility_checks && $perl_arg_index==1 && ($self->arg_descrs->[0]->flags & $func_is_static);
   ...
   foreach my $arg_attr (@$arg_attrs) {
      ...
      if ($Application::plausibility_checks) {
         ...
         if ($arg_flags & $func_arg_is_const_or_rval_ref and $sig_index >= $min) {
      ...
      if (is_object($arg_type)) {
         ...
         if ($arg_type->abstract) {
            ...
            if ($opts->builtin) {
               if ($Application::plausibility_checks and $arg_flags & $func_arg_is_const_or_rval_ref) {
...
sub prepare {
   ...
   if ($self->flags & $func_returns_list) {
      $self->wrapper_name .= ":L";
   } elsif ($self->flags & $func_returns_void) {
      $self->wrapper_name .= ":V";
   } elsif ($self->flags & $func_returns_lvalue) {
      $self->wrapper_name .= ":F";
   ...
   if ($self->flags & $func_is_method) {
      ...
      $self->flags |= $arg_flags & $func_is_static;
      ...
      if ($Application::plausibility_checks && !$internal) {
         ...
         if (defined (my $opts=$proto->cppoptions)) {
            if ($opts->builtin) {
               ...
               } elsif (not $arg_flags & $func_is_static) {
         ...
         if ($arg_flags & $func_is_static and $arg_flags != $func_is_static) {
            croak( "`static' attribute is mutually exclusive with any other argument attributes" );
         }
      }
   ...
   if ($self->flags & $func_is_static and defined($self->application) and $pkg ne $self->application->pkg) {
   ...
   $self->caller_kind = $self->flags & $func_is_static ? "stat" : $self->flags & $func_is_method ? "meth" : "free";
}
#######################################################################################
...
sub find_instance {
   ...
   foreach my $arg_descr (@{$self->arg_descrs}) {
      ...
      my $lval_flag = $arg_descr->flags & $func_arg_is_const_or_rval_ref;
      my $typeid = get_magic_typeid($args->[$arg_no], $lval_flag);
      if (defined $typeid) {
         $instance &&= $instance->{$typeid};
         if ($lval_flag) {
            if ($lval_flag >= $func_arg_is_univ_ref) {
               $instance &&= $instance->[$typeid];
      ...
      my $proto;
      if ($arg_descr->flags & $func_is_static) {
         $proto=$args->[$arg_no]->type;
      } else {
         $proto=guess_builtin_type($args->[$arg_no], (my $int_flag=$arg_descr->flags & $func_arg_is_integral));
         if ($lval_flag == $func_arg_is_lval_ref) {
...
sub new {
   ...
   push @{$self->arg_descrs}, new ArgDescr(0, $func_is_static);
...
sub new {
   my $self=&_new;
   push @{$self->arg_descrs}, new ArgDescr(0, $func_is_static), new ArgDescr(1, 0);
   $self;
}
...
sub new {
   ...
   if ($self->total_args == 2) {
      if ($self->flags & $func_returns_lvalue) {
         ...
         my $swapped_arg_descrs=[ map { new ArgDescr($_, $self->arg_descrs->[1-$_]->flags | $func_args_are_swapped) } 0..1 ];
         $self->code=sub {
            if (pop @_) {
               swap_array_elems(\@_, 0, 1);
               &{ local_array($self->arg_descrs, $swapped_arg_descrs); resolve_auto_function($self, \@_) }
            } else {
      ...
      my $is_lvalue= ($self->flags & $func_returns_lvalue) != 0;
...
sub load_private_wrapper {
   ...
   if (defined($private_wrapper_ext) &&
      ...
      $code_generation eq "shared" and
      local_scalar($move_private_instances, 1) and
      local_scalar($root->type_descr, $root->private_classes);
...
sub add_auto_function {
   ...
   if ($flags & $func_is_method and $options->{name} eq "()") {
   ...
   if (defined $regular_index) {
      ...
      check_twins($auto_func);
      {
         no warnings 'void';
         use namespaces::AnonLvalue '$is_lvalue';
         my $is_lvalue= ($flags & $func_returns_lvalue) != 0;
...
my %op_descr=( '+' => [ 'add', 0, 2 ], '-' => [ 'sub', 0, 2 ], '*' => [ 'mul', 0, 2 ], '/' => [ 'div', 0, 2 ], '%' => [ 'mod', 0, 2 ],
               ...
               '+=' => [ 'Add', $func_returns_lvalue, 2 ], '-=' => [ 'Sub', $func_returns_lvalue, 2 ], '*=' => [ 'Mul', $func_returns_lvalue, 2 ],
               '/=' => [ 'Div', $func_returns_lvalue, 2 ], '%=' => [ 'Mod', $func_returns_lvalue, 2 ], '<<=' => [ 'Lsh', $func_returns_lvalue, 2 ],
               '>>=' => [ 'Rsh', $func_returns_lvalue, 2 ], '&=' => [ 'And', $func_returns_lvalue, 2 ], '|=' => [ '_Or', $func_returns_lvalue, 2 ],
               '^=' => [ 'Xor', $func_returns_lvalue, 2 ],
               'neg' => [ 'neg', 0, 1 ], '!' => [ 'not', 0, 1 ], '~' => [ 'com', 0, 1 ], 'bool' => [ 'boo', 0, 1 ],
               '++' => [ 'inc', $func_returns_lvalue, 1 ], '--' => [ 'dec', $func_returns_lvalue, 1 ],
...
sub create_operator {
   ...
   if ($func_flags & $func_returns_lvalue) {
      # assignment version
      $_ &= ~($func_arg_is_const_or_rval_ref | $func_arg_has_anchor) for @arg_flags;
      $arg_flags[0] |= $func_arg_is_lval_ref;
   ...
   if (defined $signature) {
      ...
      if ($create_root) {
         no warnings 'void';
         use namespaces::AnonLvalue '$is_lvalue';
         my $is_lvalue= ($func_flags & $func_returns_lvalue) != 0;
...
sub bind_functions {
   ...
   foreach my $descr (splice @$func_list) {
      ...
      if (defined ($sub=create_function_wrapper($descr, $app_stash, $auto_func->total_args, $auto_func->returns))) {
         declare_lvalue($sub) if $auto_func->flags & $func_returns_lvalue;

         if ($move_private_instances) {
            no warnings 'void';
            use namespaces::AnonLvalue '$is_lvalue';
            my $inner_sub=$sub;
            my $is_lvalue= ($auto_func->flags & $func_returns_lvalue) != 0;
            $sub=sub {
               $is_lvalue;
...
use Polymake::Enum qw( func: arg_is_const_ref=0 arg_is_lval_ref=0x1 arg_is_univ_ref=0x2 arg_is_const_or_rval_ref=0x3
                             arg_is_wary=0x4 arg_has_anchor=0x8 arg_is_integral=0x10
                             is_method=0x20 is_static=0x40 returns_list=0x80 returns_void=0x100 returns_lvalue=0x200
...
sub try_merge_auto_functions {
   ...
   if (not($f1->flags & $func_is_method) &&
...
my @assoc_descr=(
   [ $assoc_helem,       "brk",           "[]",                                           $func_returns_lvalue,
     $func_arg_is_univ_ref, 0 ],
   [ $assoc_find,        "assoc_find",    [$binary_op_signature->[0], [qw(*:anchor *)]],  0,
     name => "pm::perl::find_element", include => [ "polymake/perl/assoc.h" ] ],
   [ $assoc_exists,      "exists",        [$unary_op_signature->[0],  ["", "*"]],         $func_is_method ],
   [ $assoc_delete_void, "erase",         [$unary_op_signature->[0],  [qw(& *)]],         $func_is_method ],
   [ $assoc_delete_ret,  "assoc_delete",  [$binary_op_signature->[0], [qw(*& *)]],        0,
